...
function ensureButtonsVisible() {
    ...
    buttons.forEach(buttonContainer => {
        // Buttons are now inside the card, just ensure they're visible
...
function renderButtonIcon(iconValue, position = 'left') {
    ...
    if (trimmedValue.startsWith('http') || trimmedValue.startsWith('data:') || trimmedValue.includes('.')) {
        const isGif = trimmedValue.toLowerCase().includes('.gif') || trimmedValue.includes('data:image/gif');
        if (isGif) {
            return `<video class="${className}" autoplay muted loop playsinline><source src="${trimmedValue}" type="video/mp4"></video>`;
        } else {
            return `<img class="${className}" src="${trimmedValue}" alt="icon">`;
        }
    }
...
function convertToEmbedUrl(url) {
    ...
    try {
        ...
        if (url.includes('youtube.com') || url.includes('youtu.be')) {
            ...
            if (videoId) {
                // Add parameters for better Android compatibility
                return `https://www.youtube.com/embed/${videoId}?enablejsapi=1&origin=${window.location.origin}`;
            }
        }
        
        // Facebook - Multiple formats with Android compatibility
        if (url.includes('facebook.com') || url.includes('fb.watch')) {
            ...
            if (url.includes('fb.watch/')) {
                ...
                return `https://www.facebook.com/plugins/video.php?href=${encodeURIComponent(cleanUrl)}&show_text=false&width=560&height=315&appId=`;
            }
        }
...
// Create video iframe with Android compatibility
function createVideoIframe(src, id = 'mainMedia') {
    // Enhanced iframe attributes for Android compatibility
    return `<iframe 
        id="${id}" 
        src="${src}" 
        frameborder="0" 
...
function showProductDetail(product) {
    ...
    if (allMedia[0].type === 'video') {
        mainMediaHTML = createVideoIframe(allMedia[0].src, 'mainMedia');
    } else {
    ...
    detail.innerHTML = `
        <div class="safe-frame-scrollable">
...
window.changeMainMedia = function(index) {
    ...
    if (media.type === 'video') {
        container.innerHTML = createVideoIframe(media.src, 'mainMedia');
    } else {
